...
class myMap {
    ...
    constructor(data,    mapData, tempChart, humidityChart, pollutionChart) {
        ...
        this.cityData = [{x:87, y:221, n:"Eureka", c:"Humbolt"},{x:184, y:332, n:"Sacramento", c:"Sacramento"},
            {x:143, y:365, n:"San Francisco", c:"San Francisco"},{x:167, y:395, n:"San Jose", c:"Santa Clara"},
    ...
    drawOption(){
        ...
        svgContainer.append('text')
                        ...
                        .on("click", function(d){ 
                            ...
                            that.selectedFeature = "Pollution";
                            document.getElementById("myTempSVG").style.display = "none";
                            document.getElementById("myHumiditySVG").style.display = "none";                          document.getElementById("myPollutionSVG").style.display = "block";
                            that.update();
                            })
                        .on('mouseover', function(d) {
                            //d3.select(".rectpol").style('fill', '#3EBAE8');
    ...
    drawMap() {
        ...
        gCircles.selectAll('circle')
            .on('click', function(d){
                ...
                if(that.selectedCitys[this.id] == false){
                    ...
                    d3.selectAll(".citylable#" + this.id).style("font-size", "15px").style("stroke", "red");
                    that.selectedCitys[this.id] = true;
                    that.lastCity = this.id;
                }else{
                    d3.select(this).classed("selected", false);
                    d3.selectAll(".citylable#" + this.id).style("font-size", "10px").style("stroke", "black");
        ...
        let gCTexts = svgMap.append("g").classed("gCTexts", true);
                let texts = gCTexts.selectAll("text").data(this.cityData).enter();
                texts.append("text").text((d, i) => parseInt(arrAvg[i]))
                        .attr("x", d=>d.x+129)
                        .attr("y", d=>d.y-122)
                        .attr("id", d=>d.n.replace(" ", "_"))
                        .style("fill", '#FFF');

        gCTexts.selectAll("text")
            .on('click', function(d){
                if(that.selectedCitys[this.id] == false){
                    d3.select(".gCircle#" + this.id).classed("selected", true);
                    d3.select(".citylable#" + this.id).style("font-size", "15px").style("stroke", "red");
                    that.selectedCitys[this.id] = true;
                    that.lastCity = this.id;
                }else{
                    d3.select(".gCircle#" + this.id).classed("selected", false);
                    d3.select(".citylable#" + this.id).style("font-size", "10px").style("stroke", "black");
                    that.selectedCitys[this.id] = false;
                    that.lastCity = "none";
                }
        ...
        this.tempChart.drawChart();
        this.humidityChart.drawChart();
        this.pollutionChart.drawChart();
    }
